#!/usr/bin/env python3
"""
Simple Dev Cleaner — Menú manual, colores, loadings, idioma persistente (es/en), APB.
"""
...
from cleaner import Config, RunSummary, scan, delete_from_summary
from system_info import get_system_info

console = Console()

# ── Traducciones (es / en). Persiste en config.json como "lang".
TEXTS = {
    "es": {
...
def t(config: Config, key: str, *args: Any) -> str:
    """Traduce una clave al idioma actual. Usar {}/format para placeholders."""
...
def menu_principal(config: Config) -> str:
    ...
    console.print()
    try:
        return Prompt.ask(f"[bold]{t(config, 'prompt_option')}[/]", choices=["0", "1", "2", "3", "4", "5"], default="0")
    except Exception:
        return "0"
...
def run_limpiar(config: Config) -> None:
    ...
    try:
        if not Confirm.ask(f"[yellow]{t(config, 'confirm_delete')}[/]\n{t(config, 'confirm_yes_no')}", default=False):
...
def run_configurar(config: Config) -> None:
    ...
    while True:
        ...
        console.print()
        try:
            op = Prompt.ask(t(config, "config_prompt"), choices=["0", "1", "2", "3", "4", "5"], default="0")
        except Exception:
            op = "0"
        if op == "0":
            break
        if op == "1":
            ...
            if not config.scan_dirs:
                ...
                for i, d in enumerate(config.scan_dirs, 1):
                    exists = f"[green]✓ {t(config, 'exists')}[/]" if Path(d).is_dir() else f"[red]{t(config, 'not_exists')}[/]"
            ...
            try:
                raw = (Prompt.ask(t(config, "hours_prompt", config.unused_hours), default=str(config.unused_hours)) or str(config.unused_hours)).strip()
                h = int(raw)
            except ValueError:
            ...
            try:
                choice = (Prompt.ask(t(config, "lang_prompt"), choices=["1", "2"], default="1") or "1").strip()
            except Exception:
                choice = "1"
            if choice == "2":
                config.lang = "en"
...
def main() -> None:
    ...
    while True:
        ...
        try:
            ...
            console.print("[dim]Si se repite, revisá config.json o borrá history.json y probá de nuevo.[/]")
